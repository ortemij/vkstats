...
if(LANG == undefined) {
	...
	var out = {
		...
		getNoteContent: function (top) {
			var value = "== " + LANG.appName + " " + VERSION + " ==\n\n";
			value += "[[club21792535|vkontakte-stats]]\n\n";
			value += "{|\n";
			value += "|-\n";
			value += "! ";
			value += "!! " + LANG.nameCol;
			value += "!! " + LANG.numberOfMessagesCol;
			value += "!! " + LANG.sentCol;
			value += "!! " + LANG.receivedCol + "\n";
			for (var i = 0; i < ((top != undefined) ? Math.min(f.length, top) : f.length); i++) {
				value += "|-\n";
				value += "| " + (i+1) + "\n";
				value += "| [" + f[i].href + "|" + f[i].name + "]\n";
				value += "| " + f[i].count + "\n";
				value += "| " + f[i].count_outgoing + "\n";
				value += "| " + f[i].count_incoming + "\n";
			}
			value += "|}\n";
		...
		note: function (top) {
			Ajax.Get({
				url: '/notes.php?act=new',
				onDone: function (ao, rt) {
					var div = ce('div', {}, {display: 'none'});
					div.innerHTML = rt;
					document.body.appendChild(div);

					ge('post').value = out.getNoteContent(top);
					ge('title').value = LANG.appName + " " + VERSION + ": TOP " + top;
					//ge('editPost').target = '_blank';

					ge('status_export').value = 1;
					ge('privacy_note').value = 0;
					ge('privacy_notecomm').value = 0;

					ge('editPost').action = '/notes.php?act=new';
					ge('editPost').submit();
				}
			});
		},
		...
		generate: function () {
			...
			ge('content').innerHTML += LANG.exportByMessages + ' <a href="#" onclick="javascript: out.note(10);">TOP 10</a> | <a href="#" onclick="javascript: out.note(50);">TOP 50</a> | <a href="#" onclick="javascript: out.note();">ALL</a>';
